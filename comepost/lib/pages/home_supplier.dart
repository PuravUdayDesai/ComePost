...
class SupplierClass {
  ...
  bool index;
  String description;
  SupplierClass(this.id, this.date, this.dryWaste, this.wetWaste, this.index,
      this.description, this.dateString);
  SupplierClass.toPost(this.id, this.date, this.dryWaste, this.wetWaste,
...
class _HomeSupplierState extends State<HomeSupplier> {
  ...
  getData() async {
    ...
    for (int i = 0; i < response1.length; i++) {
      ...
      SupplierClass obj =
          new SupplierClass(_id, _name, _phone, _email, _regd, _desc, "");
      suppliers.add(obj);
    }
    //print("dekh le!");
    return suppliers;
  ...
  addData() async {
    ...
    SupplierClass s1 = new SupplierClass(
        CurrentUser.id,
                    ...
                    Form(
                      ...
                      child: Column(children: <Widget>[
                        TextFormField(
                          ...
                          validator: (v) => validateWaste(v)
                              ? null
                              : AppLocalizations.of(context)
                                  .translate('addPopUpExecMsg'),
                        ),
                        TextFormField(
                                  ...
                                  Navigator.of(context).push(
                                    MaterialPageRoute(
                                      builder: (_) => DisplayImage(
                                          imageFile: tmpFile, user: 'Supplier'),
                                    ),
                                  );
                                }
                              },
    ...
    return PopupMenuButton<int>(
        ...
        itemBuilder: (c) => <PopupMenuItem<int>>[
              PopupMenuItem<int>(
                value: 1,
                child:
                    Text(AppLocalizations.of(context).translate('optionsT1')),
              ),
              PopupMenuItem<int>(
                value: 2,
                child:
                    Text(AppLocalizations.of(context).translate('optionsT2')),
              ),
            ]);
  }
...
class DataSearch extends SearchDelegate<String> {
  ...
  Widget buildSuggestions(BuildContext context) {
    ...
    final List<SupplierClass> suggestionList = query.isEmpty
        ? suppliers
        : suppliers
            .where((p) =>
                p.date.startsWith(query) ||
                  ...
                  child: ListTile(
                      ...
                      title: RichText(
                        ...
                        text: suggestionList[index].date.substring(
                            0,
                            query.length >= suggestionList[index].date.length
                                ? suggestionList[index].date.length
                                : query.length),
                        style: TextStyle(
                          color: Colors.black,
                          fontWeight: FontWeight.bold,
                        ...
                        children: [
                          TextSpan(
                            ...
                            style: TextStyle(
                                color:
                                    suggestionList[index].date.startsWith(query)
                                        ? Colors.grey
                                        : Colors.black),
                          ),
                        ],
                      ...
                      trailing: IconButton(
                        icon: suggestionList[index].index
