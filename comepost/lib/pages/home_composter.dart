...
class ComposterClass2 {
  ...
  ComposterClass2.addCompost(this.prices, this.weights, this.descriptions,
      this.grades, this.categorys, this.dateString);

  ComposterClass2(this.su_id, this.date, this.dryWaste, this.wetWaste,
      this.descriptions, this.dateString);
  ...
  factory ComposterClass2.fromJson(Map<String, dynamic> json) {
    ...
    return ComposterClass2(
        1, _date, _dryWaste, _wetWaste, '', _date.toString());
  }
...
class _HomeComposterState extends State<HomeComposter> {
  ...
  Future<ComposterClass2> createRecord(dataa) async {
    var response1 = await ApiCall.updateRecord(
        Uri.GET_SUPPLIER +
            "/product/sub?composterId=${CurrentUser.id}&searchDate=${ddate}",
        dataa);
  ...
  Future<List<ComposterClass>> getData(date) async {
    var response1 = await ApiCall.getDataFromApi(
        Uri.GET_SUPPLIER + "/display?date=" + date);
    ...
    for (int i = 0; i < response1.length; i++) {
      ...
      ComposterClass obj = new ComposterClass(
          _id,
          _name,
          _phone,
          _email,
      ...
      children: [
        Card(
            ...
            child: ListTile(
              ...
              onTap: () {
                buyWaste(obj.name, obj.dryWaste, obj.wetWaste, obj.id,
                    obj.description);
              },
              isThreeLine: true,
                    ...
                    Form(
                      ...
                      child: Column(children: <Widget>[
                        TextFormField(
                          ...
                          decoration: InputDecoration(
                            labelText: AppLocalizations.of(context)
                                .translate('addPopUpWCol'),
                            hintText: AppLocalizations.of(context)
                                    .translate('addPopUpWHint') +
                                " (max $wet kg).",
                        ...
                        TextFormField(
                          ...
                          decoration: InputDecoration(
                            labelText: AppLocalizations.of(context)
                                .translate('addPopUpDCol'),
                            hintText: AppLocalizations.of(context)
                                    .translate('addPopUpDHint') +
                                " (max $dry kg).",
                            icon: Icon(Icons.add_shopping_cart),
                          ),
                          validator: (v) {
            ...
            child: new Wrap(
              children: <Widget>[
                new ListTile(
                    ...
                    title: new Text(
                        AppLocalizations.of(context).translate('signUpF1') +
                            ": " +
                            obj.name),
                    onTap: () => {}),
                new ListTile(
                  ...
                  title: new Text(
                      AppLocalizations.of(context).translate('addPopUpDCol') +
                          ": " +
                          obj.dryWaste.toString()),
                  onTap: () => {},
                ),
                new ListTile(
                  //leading: new Icon(Icons.videocam),
                  title: new Text(
                      AppLocalizations.of(context).translate('addPopUpWCol') +
                          ": " +
                          obj.wetWaste.toString()),
                  onTap: () => {},
                ),
                new ListTile(
                  //leading: new Icon(Icons.videocam),
                  title: new Text(
                      AppLocalizations.of(context).translate('signUpF3') +
                          ": " +
                ...
                new ListTile(
                  //leading: new Icon(Icons.videocam),
                  title: new Text(
                      AppLocalizations.of(context).translate('signUpF4') +
                          ": " +
                          obj.emailId.toString()),
                  onTap: () => {},
                ),
                new ListTile(
                  //leading: new Icon(Icons.videocam),
                  title: new Text(
                      AppLocalizations.of(context).translate('signUpF5') +
                          ": " +
                          obj.registrationNumber.toString()),
                  onTap: () => {},
                ),
                new ListTile(
                  //leading: new Icon(Icons.videocam),
                  title: new Text(
                      AppLocalizations.of(context).translate('lastAddDate') +
                          ": " +
                          obj.dateString.toString()),
                  onTap: () => {},
                ),
                new ListTile(
                  //leading: new Icon(Icons.videocam),
                  title: new Text(
                      AppLocalizations.of(context).translate('signUpF7') +
                          ": " +
                          obj.city.toString()),
                  onTap: () => {},
                ),
                new ListTile(
                  //leading: new Icon(Icons.videocam),
                  title: new Text(
                      AppLocalizations.of(context).translate('signUpF6') +
                          ": " +
                          obj.sstate.toString()),
                  onTap: () => {},
                ),
...
//dynamic contactNumber,emailId,registrationNumber,date_time,sstate,city,latitude,longitude;
  ...
  addData(su_id, description) async {
    ...
    ComposterClass2 c1 = new ComposterClass2(
        su_id,
        DateTime.now().toString().substring(0, 19),
        double.parse(dryController.text),
        double.parse(wetController.text),
        description,
        "");
    print(c1.toMap());
  ...
  addCompostRecord(data) async {
    var r =
        await ApiCall.createRecord(Uri.GET_COMPOSTER + '/compost/add', data);
  }
...
class DataSearch extends SearchDelegate<String> {
  ...
  Widget buildSuggestions(BuildContext context) {
    ...
    final List<ComposterClass> suggestionList = query.isEmpty
        ? composters
        : composters
            .where((p) =>
                p.name.startsWith(query) ||
                p.dryWaste.toString().startsWith(query) ||
                p.contactNumber.startsWith(query) ||
                p.wetWaste.toString().startsWith(query) ||
                p.dateString.contains(query) ||
                p.emailId.startsWith(query) ||
                p.city.startsWith(query) ||
                p.sstate.startsWith(query))
            .toList();
    return ListView.builder(
              ...
              Card(
                  ...
                  child: ListTile(
                      ...
                      subtitle: Text(
                          "\n\t\t\t\t" +
                              AppLocalizations.of(context)
                                  .translate('cardTxtOne') +
                              " = " +
                              suggestionList[index].dryWaste.toString() +
                              " Kg.\t\t\t\t" +
                              AppLocalizations.of(context)
                                  .translate('cardTxtTwo') +
                              " = " +
                              suggestionList[index].wetWaste.toString() +
                              " Kg.",
                          style: TextStyle(
                            color: Colors.brown,
                            fontSize: 13,
